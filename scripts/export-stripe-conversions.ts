 ...
 * 
 * Uso:
 *   npx ts-node scripts/export-stripe-conversions.ts
 * 
 * Requiere:
 *   - STRIPE_PRIVATE_KEY en .env o como variable de entorno
 */
...
async function getAllSubscriptionsWithGclid(): Promise<ConversionData[]> {
  ...
  while (hasMore) {
    ...
    for (const subscription of subscriptions.data) {
      ...
      if (gclid) {
        ...
        console.log(`  ✅ ${customer.email} - ${gclid.substring(0, 20)}...`);
      }
    }
...
function generateGoogleAdsCSV(conversions: ConversionData[]): string {
  ...
  const rows = conversions.map(conv => [
    conv.gclid,
    'YQH - Registro - Thanks', // Nombre de la conversión en Google Ads
    conv.conversionTime,
...
function generateJSON(conversions: ConversionData[]): string {
